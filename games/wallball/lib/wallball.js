...
(function() {
  ...
  this.Ball = (function(_super) {
    ...
    Ball.prototype.init = function() {
      this.r.x = Game.instance.paddle.r.x;
      this.r.y = Game.height - Game.instance.paddle.padding - Game.instance.paddle.bb_height - this.config.size;
      this.v.x = 0;
    ...
    Ball.prototype.draw = function() {
      ...
      if (this.r.y >= Game.height - this.size - this.tol) {
        if (this.r.y <= (Game.instance.paddle.r.y + Game.instance.paddle.height) && Math.abs(this.r.x - Game.instance.paddle.r.x) <= Game.instance.paddle.size) {
          Game.instance.paddle.remove_check(this);
        } else {
          Gamescore.lives -= 1;
  ...
  this.Wallball = (function(_super) {
    ...
    function Wallball(config) {
      ...
      this.scoretxt = this.g.append("text").text("").attr("stroke", "#222").attr('stroke-width', '2px').attr("fill", "#F90").attr("font-size", "42").attr("x", "20").attr("y", "80").attr('font-family', 'arial').attr('font-weight', 'bold');
      this.lives = this.g.append("text").text("").attr("stroke", "#222").attr('stroke-width', '2px').attr("fill", "#F90").attr("font-size", "42").attr("x", "20").attr("y", "40").attr('font-family', 'arial').attr('font-weight', 'bold');
    ...
    Wallball.prototype.spawn_ball = function() {
      var dur, ready, _ref,
        _this = this;
      if (!(this.ball === null || ((_ref = this.ball) != null ? _ref.is_removed : void 0))) {
        return;
      }
      this.ball = null;
      this.svg.style("cursor", "none");
      ready = this.g.append("text").text("GET READY").attr("stroke", "none").attr("fill", "#F90").attr("font-size", "36").attr("x", Game.width / 2 - 105).attr("y", Game.height / 2 + 20).attr('font-family', 'arial').attr('font-weight', 'bold').attr('opacity', 0);
      dur = 1000;
      ready.transition().duration(dur).style("opacity", 1).transition().duration(dur).style('opacity', 0).remove().each('end', function() {
        _this.ball = Factory.spawn(Ball);
        return _this.ball.start();
      });
    };
    ...
    Wallball.prototype.start = function() {
      var go, how, title,
        _this = this;
      title = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "48").attr("x", Game.width / 2 - 135).attr("y", 90).attr('font-family', 'arial').attr('font-weight', 'bold');
      title.text("WALLBALL");
      go = this.g.append("text").text("").attr("stroke", "none").attr("fill", "#FF2").attr("font-size", "36").attr("x", Game.width * 0.5 - 60).attr("y", Game.height / 2 + 130).attr('font-family', 'arial').attr('font-weight', 'bold').style("cursor", "pointer").text("START");
      how = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "18").attr("x", Game.width / 2 - 170).attr("y", Game.height - 100).attr('font-family', 'arial').attr('font-weight', 'bold').style("cursor", "pointer").text("Use mouse / drag to control movement");
