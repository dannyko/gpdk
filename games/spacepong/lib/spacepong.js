...
(function() {
  ...
  $z.Ball = (function(_super) {
    ...
    Ball.prototype.init = function() {
      this.v.x = 0;
      this.v.y = -this.speed;
      this.r.x = $z.Game.paddle.r.x + 2 * Math.random() - 1;
      this.r.y = $z.Game.height - $z.Game.paddle.padding - $z.Game.paddle.bb_height - this.config.size - this.tol;
  ...
  $z.Paddle = (function(_super) {
    ...
    function Paddle(config) {
      ...
      this.image = this.g.append("image").attr("xlink:href", $z.Paddle.image_url).attr("x", -this.size - this.overshoot).attr("y", -this.height).attr("width", this.size * 2 + this.overshoot * 2).attr("height", this.height * 2);
      this.start();
    }
  ...
  $z.Ship = (function(_super) {
    ...
    Ship.prototype.init = function() {
      ...
      this.speed = $z.Ship.speed[this.difficulty];
      this.size = $z.Ship.size[this.difficulty];
      this.image.remove();
      w = this.size;
      h = this.size;
    ...
    Ship.prototype.draw = function() {
      ...
      if (this.r.x > ($z.Game.width - this.size)) {
        this.r.x = $z.Game.width - this.size;
      }
    ...
    Ship.prototype.remove = function(quietSwitch) {
      ...
      if (quietSwitch == null) {
        quietSwitch = $z.Gamescore.lives < 0;
      }
    ...
    Ship.prototype.remove_check = function(element) {
      ...
      if (element.name === 'Ball') {
        ...
        if ($z.Gamescore.lives >= 0) {
          for (i = _i = 0, _ref = $z.Ship.increment_count[this.difficulty]; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
            $z.Gamescore.increment_value();
          }
        }
        $z.Game.instance.text();
        if (old_count < $z.Spacepong.ball_count()) {
          $z.Game.instance.spawn_ball('MULTIBALL UP');
        }
        this.remove();
        return true;
      } else {
  ...
  $z.Spacepong = (function(_super) {
    ...
    Spacepong.ball_count = function() {
      ...
      if ($z.Gamescore.value < 1000) {
        count = 1;
      }
      if ((1000 <= (_ref = $z.Gamescore.value) && _ref < 5000)) {
        count = 2;
      }
      if ((5000 <= (_ref1 = $z.Gamescore.value) && _ref1 < 10000)) {
        count = 3;
      }
      if (10000 <= $z.Gamescore.value) {
        count = 4;
      }
      return count;
    };
    ...
    Spacepong.prototype.start = function() {
      var go, how, title;
      title = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "48").attr("x", $z.Game.width / 2 - 320).attr("y", 90).attr('font-family', 'arial').attr('font-weight', 'bold').text("SPACEPONG");
      how = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "18").attr("x", $z.Game.width / 2 - 320).attr("y", $z.Game.height / 2 + 130).attr('font-family', 'arial').attr('font-weight', 'bold').text("drag, move the mouse, or use left/right arrow-keys to control the paddle");
      go = this.g.append("text").text("").attr("stroke", "none").attr("fill", "#FF2").attr("font-size", "36").attr("x", $z.Game.width * 0.5 - 60).attr("y", $z.Game.height - 100).attr('font-family', 'arial').attr('font-weight', 'bold').style("cursor", "pointer").text("START");
