...
(function() {
  ...
  $z.Drone = (function(_super) {
    ...
    function Drone(config) {
      ...
      this.set_param();
      this.max_speed = $z.Drone.max_speed;
      this.invincible = false;
      this.energy = this.config.energy || 10;
      this.image.remove();
      this.g.attr("class", "drone");
      this.image = this.g.insert("image", ":first-child").attr("xlink:href", $z.Drone.url).attr("x", -this.size).attr("y", -this.size).attr("width", this.size * 2).attr("height", this.size * 2);
    ...
    Drone.prototype.set_param = function() {
      ...
      this.param.q = this.collision ? this.root.charge * (1 + $z.Gamescore.value / 1000) : 1e-6;
    ...
    Drone.prototype.remove = function() {
      ...
      if ($z.Gamescore.lives >= 0) {
        ...
        this.g.transition().duration(dur).ease('poly(0.5)').style("opacity", "0").each('end', function(d) {
          d.is_removed = true;
          return d.overlay.style('opacity', 0);
        });
        scaleSwitch = false;
        if (scaleSwitch) {
      ...
      if ($z.Game.instance.element.every(function(d) {
        return !d.collision;
      })) {
    ...
    Drone.prototype.offscreen = function() {
      ...
      dx = $z.Game.width * 0.5 - this.r.x;
      dy = $z.Game.height * 0.5 - this.r.y;
      d = Math.sqrt(dx * dx + dy * dy);
      scale = 0.01 * this.max_speed / d;
      if (Math.abs(dx) > $z.Game.width * 0.5 - this.size) {
        this.v.x += scale * dx;
      }
  ...
  $z.Dronewar = (function(_super) {
    ...
    Dronewar.prototype.level = function() {
      ...
      if ($z.Gamescore.lives < 0 || $z.Physics.off) {
        return;
      }
      drone_increment = 1;
      if (!(this.N >= this.maxN)) {
      ...
      for (i = _i = 0, _ref = this.N; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        if (!($z.Gamescore.lives < 0 || $z.Physics.off)) {
          newAttacker = $z.Factory.spawn($z.Drone, drone_config);
        }
        this.element.push(newAttacker);
        this.element[i].r.x = $z.Game.width * 0.5 + (Math.random() - 0.5) * 0.5 * $z.Game.width;
        this.element[i].r.y = $z.Game.height * 0.1 + Math.random() * 0.1 * $z.Game.height;
    ...
    Dronewar.prototype.stop = function() {
      ...
      this.message('GAME OVER', (function(_this) {
        return function() {
          return Dronewar.__super__.stop.call(_this);
        };
      })(this));
    };
    ...
    Dronewar.prototype.start = function() {
      ...
      this.root.draw();
      title = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "48").attr("x", $z.Game.width / 2 - 150).attr("y", 60).attr('font-family', 'arial').attr('font-weight', 'bold');
      title.text("DRONEWAR");
      ...
      viper = this.g.append("text").text("").attr("stroke", "none").attr("fill", "white").attr("font-size", "32").attr("x", $z.Game.width / 2 - 350).attr("y", $z.Game.height / 4 + 200).attr('font-family', 'arial').attr('font-weight', 'bold').style("cursor", "pointer");
      viper.text("VIPER");
    ...
    Dronewar.prototype.text = function() {
      ...
      return this.lives.text('ENERGY: ' + $z.Gamescore.lives);
    };
  ...
  $z.Root = (function(_super) {
    ...
    function Root(config) {
      ...
      this.dragspin = __bind(this.dragspin, this);
      this.spin = __bind(this.spin, this);
      this.redraw_interp = __bind(this.redraw_interp, this);
    ...
    Root.prototype.apply_limits = function(xy) {
      return [Math.min(Math.max(this.bb_width, xy[0]), $z.Game.width - this.bb_width), Math.min(Math.max(this.bb_height, xy[1]), $z.Game.height - this.bb_height)];
    };
    ...
    Root.prototype.fire = function() {
      ...
      if (this.lastfire === void 0 || this.lastfire > $z.Physics.timestamp) {
        this.lastfire = $z.Physics.timestamp;
        return;
      }
      if (!(($z.Physics.timestamp - this.lastfire) >= this.wait)) {
        return;
      }
      this.lastfire = $z.Physics.timestamp;
      this.shoot();
